...
class BasicProcessor:
    ...
    def get_new_dict(tag_text, tag_type, tag_sub_type="", is_inline=True, tag_extension="", new_uuid=None):
        ...
        new_dict[Config.KEY_TEXT] = tag_text
        new_dict[Config.KEY_TYPE] = tag_type
        new_dict[Config.KEY_SUB_TYPE] = tag_sub_type
        new_dict[Config.KEY_INLINE_FLAG] = is_inline
        new_dict[Config.KEY_EXTENSION] = tag_extension
        if new_uuid is None:
            new_uuid = UUID.get_new_uuid()
            new_dict[Config.KEY_UUID] = new_uuid
